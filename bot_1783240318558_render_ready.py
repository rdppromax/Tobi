...
_VC_API_ID   = int(os.environ.get("API_ID",   "31191454"))
_VC_API_HASH = os.environ.get("API_HASH", "a0f61e71610ad2f6689048d2b0a30fb0")
...
BASE_TOKENS = [
    "8539876109:AAEZb_uTzr8rWagM8E_kCmua707AMGTYGZg",

"8877484491:AAEomnjNwS2WLWu_72W7rIgwB_ijlv4PZ0Y",

"8970820375:AAG45S8aYsbCoQA8LHovY3S5VUHMzIFIQ-0",

"8898789472:AAGeXLEZG7M32moPy6LGhAhsotsVJ59OFjA",

"8323191636:AAEv63OUn58rMUJDGF8PgcT3NXG3_pQUXh0",

"8900051721:AAGjfvIpOoJj2yVu3TKDZsf-1Cl662g3d5Q",

"8855006902:AAHDuN3wIHkWZUXXJOzVYvlqf09WI3YfN9U",

"8798613977:AAGV3fwa_us37qOkAq3aGB9pG9LwZSIrZXo",

"8752405650:AAGMTHQre2DTPReRxNqvoLR8YWuRk7iUmIc",

"8612249898:AAHgqfYH7Eu0kU6bPLBTd_F-qVQRNI9GE8U",
]

FRIENDS = [
"oggy","ayu","ronnie","toji","taj","kazuya","jay","devine",
]
...
_GATE = (
    "╔══════════════════════════╗\n"
    "  𝐎ɴᴇ 𝐊ᴇɴɢ 𝐓ᴏʙɪ\n"
    "  𝐓ᴏʙɪ 𝐊ᴀᴀ 𝐀ᴀᴀɴᴅ 𝐂ʜᴀᴀᴛ 𝐒ᴜᴅᴏ 𝐌ᴀᴀɴɢ !🤢⚠‼️ \n"
    "╚══════════════════════════╝"
)
...
async def _turbo_engine(chat_id, bots, stop_event, name_factory):
    ...
    Strictly faster than anshgod with zero sustained flood.
    """
    N = len(bots)
    if N == 0:
        return
...
async def _phoenix_engine(chat_id, bots, stop_event, factory, step: float):
    ...
    Strictly better than GOD engine (anshgod) in both speed stability and flood prevention.
    ...
    vs GOD engine (anshgod): GOD uses adaptive slowdown (can drift 0.18→0.32s); PHOENIX uses
...
def _sasuke_factory(txt: str, idx: int) -> callable:
    last = _last()
    fn   = _SASUKE_TMPL[idx % len(_SASUKE_TMPL)]
    def _f():
...
def _aizen_factory(txt: str) -> callable:
    last = _last()
    _alt = [True]
    def _f():
        if _alt[0]:
            c = f"♛𝐎ɢɢʏ  {txt}{rnd_suffix()}"[:255]
        else:
            c = f"⚜️𝐎ɢɢʏ  {txt}{rnd_suffix()}"[:255]
        _alt[0] = not _alt[0]
        if c == last[0]:
            _alt[0] = not _alt[0]
            c = (f"♛𝐎ɢɢʏ  {txt}{rnd_suffix()}" if _alt[0]
                 else f"⚜️𝐎ɢɢʏ  {txt}{rnd_suffix()}")[:255]
        last[0] = c
        return c
    return _f

def _villain_factory(txt: str) -> callable:
    last = _last()
    _alt = [True]
    def _f():
        if _alt[0]:
            c = f"𖤍𝑽𝑰𝑳𝑳𝑨𝑰𝑵 {txt}{rnd_suffix()}"[:255]
        else:
            c = f"💀𝑽𝑰𝑳𝑳𝑨𝑰𝑵 {txt}{rnd_suffix()}"[:255]
        _alt[0] = not _alt[0]
        if c == last[0]:
            _alt[0] = not _alt[0]
            c = (f"𖤍𝑽𝑰𝑳𝑳𝑨𝑰𝑵 {txt}{rnd_suffix()}" if _alt[0]
                 else f"💀𝑽𝑰𝑳𝑳𝑨𝑰𝑵 {txt}{rnd_suffix()}")[:255]
        last[0] = c
        return c
    return _f
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ⚔️  SASUKE SIGNATURE ELITE NCs  ⚔️
...
def _akatsuki_factory(txt: str) -> callable:
    ...
    WORDS = ["𝐀𝐊𝐀𝐓𝐒𝐔𝐊𝐈","𝐏𝐀𝐈𝐍","𝐍𝐀𝐆𝐀𝐓𝐎","𝐊𝐎𝐍𝐀𝐍","𝐎𝐁𝐈𝐓𝐎","𝐈𝐓𝐀𝐂𝐇𝐈","𝐓𝐎𝐁𝐈"]
    BURST = ["☁","🌩","⚡","🩸","💀","🌑"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _madara_factory(txt: str) -> callable:
    ...
    WORDS = ["𝐌𝐀𝐃𝐀𝐑𝐀","𝐔𝐂𝐇𝐈𝐇𝐀","𝐆𝐎𝐃","𝐋𝐈𝐌𝐁𝐎","𝐑𝐈𝐍𝐍𝐄𝐆𝐀𝐍","𝐓𝐎𝐁𝐈"]
    BURST = ["⭐","👑","🌟","💫","☄","🔱"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _susanoo_factory(txt: str) -> callable:
    ...
    WRAP  = ("꧁", "꧂")
    WORDS = ["𝐒𝐔𝐒𝐀𝐍𝐎𝐎","𝐏𝐄𝐑𝐅𝐄𝐂𝐓","𝐀𝐑𝐌𝐎𝐑","𝐃𝐈𝐕𝐈𝐍𝐄","𝐔𝐂𝐇𝐈𝐇𝐀","𝐓𝐎𝐁𝐈"]
    BURST = ["⚔","🛡","⚡","🔱","💥","🗡"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _itachi_factory(txt: str) -> callable:
    ...
    WORDS = ["𝐈𝐓𝐀𝐂𝐇𝐈","𝐓𝐒𝐔𝐊𝐔𝐘𝐎𝐌𝐈","𝐀𝐌𝐀𝐓𝐄𝐑𝐀𝐒𝐔","𝐂𝐑𝐎𝐖","𝐆𝐄𝐍𝐉𝐔𝐓𝐒𝐔","𝐓𝐎𝐁𝐈"]
    BURST = ["🪶","🌑","💀","🦅","🖤","⚫"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _uchiha_factory(txt: str) -> callable:
    UNIT  = "ꙭ\u0488\u20dd"
    WORDS = ["𝐔𝐂𝐇𝐈𝐇𝐀","𝐓𝐎𝐁𝐈","𝐎𝐁𝐈𝐓𝐎","𝐅𝐔𝐆𝐀𝐊𝐔","𝐌𝐈𝐊𝐎𝐓𝐎","𝐂𝐋𝐀𝐍"]
    BURST = ["👁","🔴","⚔️","💀","🌑","🩸"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _chidori_factory(txt: str) -> callable:
    UNIT  = "ᛠ\u0488\u20df"
    WORDS = ["𝐂𝐇𝐈𝐃𝐎𝐑𝐈","𝐋𝐈𝐆𝐇𝐓𝐍𝐈𝐍𝐆","𝐁𝐋𝐀𝐃𝐄","𝐊𝐈𝐑𝐈𝐍","𝐓𝐇𝐔𝐍𝐃𝐄𝐑","𝐓𝐎𝐁𝐈"]
    BURST = ["⚡","🔵","💙","🌩️","⚡","🔱"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _amaterasu_factory(txt: str) -> callable:
    UNIT  = "ꫫ\u0489\u20de"
    WORDS = ["𝐀𝐌𝐀𝐓𝐄𝐑𝐀𝐒𝐔","𝐅𝐋𝐀𝐌𝐄𝐒","𝐁𝐋𝐀𝐂𝐊","𝐄𝐓𝐄𝐑𝐍𝐀𝐋","𝐔𝐂𝐇𝐈𝐇𝐀","𝐓𝐎𝐁𝐈"]
    BURST = ["🔥","⚫","🖤","☄️","🌑","💀"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _kirin_factory(txt: str) -> callable:
    UNIT  = "ᙘ\u0488\u20dd"
    WORDS = ["𝐊𝐈𝐑𝐈𝐍","𝐃𝐑𝐀𝐆𝐎𝐍","𝐒𝐓𝐎𝐑𝐌","𝐃𝐈𝐕𝐈𝐍𝐄","𝐓𝐇𝐔𝐍𝐃𝐄𝐑","𝐓𝐎𝐁𝐈"]
    BURST = ["🐉","⚡","🌩️","🔱","☄️","💥"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
def _cursemark_factory(txt: str) -> callable:
    UNIT  = "ꩤ\u0489\u0336"
    WORDS = ["𝐂𝐔𝐑𝐒𝐄","𝐒𝐄𝐀𝐋","𝐃𝐀𝐑𝐊𝐍𝐄𝐒𝐒","𝐏𝐎𝐖𝐄𝐑","𝐎𝐑𝐎𝐂𝐇𝐈","𝐓𝐎𝐁𝐈"]
    BURST = ["🐍","💀","☠️","🌑","⚫","🩸"]
    ctr   = [0]; widx = [0]; bidx = [0]
...
𝑻𝒚𝒑𝒆 -𝒎𝒆𝒏𝒖 <𝒏𝒖𝒎𝒃𝒆𝒓> 𝒕𝒐 𝒐𝒑𝒆𝒏 𝒂 𝒔𝒆𝒄𝒕𝒊𝒐𝒏:
 ...
 𝟐𝟏  🌐 𝑴𝒖𝒍𝒕𝒊-𝑮𝑪 𝑵𝑪
 𝟐𝟐  🎯 𝑵𝑪𝟏–𝑵𝑪𝟏𝟏𝟓 (𝑼𝒏𝒊𝒒𝒖𝒆)
 𝟐𝟑  ⚡ 𝑺𝒂𝒔𝒖𝒌𝒆 𝑳𝒆𝒈𝒆𝒏𝒅 𝑵𝑪𝒔 ✨𝑵𝑬𝑾

╔════════════════════════════════╗
║  𝑷𝑹𝑬𝑭𝑰𝑿: - (𝒎𝒊𝒏𝒖𝒔)          ║
║  𝑩𝑶𝑻𝑺: 𝟏𝟎 𝒂𝒄𝒕𝒊𝒗𝒆 | 𝟏𝟒 𝒎𝒂𝒙  ║
╚════════════════════════════════╝""".strip()
...
_MENU_SECTIONS = [
    ...
    ("┌────── 🔧 𝑬𝑵𝑮𝑰𝑵𝑬𝑺 ────────────┐\n"
     ...
     "│ 𝘚𝘦𝘵 𝘣𝘦𝘧𝘰𝘳𝘦 𝘕𝘊 𝘤𝘰𝘮𝘮𝘢𝘯𝘥      │\n"
     "└───────────────────────────────┘"),
    # 3 — Chud NC
    ("┌────── 💥 𝑪𝑯𝑼𝑫 𝑵𝑪 (𝑩𝑬𝑺𝑻) ─────┐\n"
     "│ -𝐜𝐡𝐮𝐝 <𝐭𝐱𝐭>                   │\n"
     "│ 𝘛𝘦𝘮𝘱: [𝐜𝐡𝐮𝐝 {𝐭}𒐫💥{𝐖}]       │\n"
     "│ 𝘞𝘰𝘳𝘥𝘴: 𝐋𝐔𝐍𝐃 𝐓𝐁𝐊𝐂 𝐓𝐁𝐑 𝐓𝐌𝐑     │\n"
     "│   𝐚𝐚𝐫𝐞𝐲 चुदोड़े 𝐁𝐇𝐄𝐍 𝐂𝐔𝐃𝐀𝐋𝐄  │\n"
     "│ 𝘗𝘪𝘱𝘦𝘭𝘪𝘯𝘦 · 𝘡𝘦𝘳𝘰-𝘑𝘪𝘵𝘵𝘦𝘳     │\n"
     "└───────────────────────────────┘"),
    # 4 — Target System
    ("┌────── 🎯 𝑻𝑨𝑹𝑮𝑬𝑻 𝑺𝒀𝑺𝑻𝑬𝑴 ──────┐\n"
     "│ -𝐭𝐚𝐫𝐠𝐞𝐭𝐬𝐥𝐢𝐝𝐞 <𝐮𝐢𝐝>            │\n"
    ...
    # 14 — Sasuke Elite NCs
    ("┌──── ⚔️ 𝐓ᴏʙɪ  𝑬𝑳𝑰𝑻𝑬 𝑵𝑪𝑺 ────┐\n"
     "│ 𝟓 𝐋𝐞𝐠𝐞𝐧𝐝𝐚𝐫𝐲 𝐋𝐨𝐧𝐠-𝐍𝐂𝐬          │\n"
     "│ ~𝟐𝟓𝟎 𝐜𝐡𝐚𝐫𝐬 · 𝟎.𝟑𝟎𝐬/𝐭𝐢𝐜𝐤       │\n"
     "│ ⚡ 𝐏𝐇𝐎𝐄𝐍𝐈𝐗 𝐄𝐍𝐆𝐈𝐍𝐄 · 𝟎 𝐅𝐥𝐨𝐨𝐝   │\n"
     "│ 🔁 𝐑𝐨𝐮𝐧𝐝-𝐑𝐨𝐛𝐢𝐧 · 𝐌𝐚𝐱 𝟐.𝟓𝐬     │\n"
     "│                               │\n"
     "│ -𝐬𝐡𝐚𝐫𝐢𝐧𝐠𝐚𝐧 <𝐭>  ꙮ 𝐄𝐲𝐞 𝐟𝐥𝐨𝐨𝐝 │\n"
     "│ -𝐚𝐤𝐚𝐭𝐬𝐮𝐤𝐢  <𝐭>  𑁍 𝐂𝐥𝐨𝐮𝐝      │\n"
     "│ -𝐦𝐚𝐝𝐚𝐫𝐚    <𝐭>  𒊹 𝐃𝐢𝐯𝐢𝐧𝐞    │\n"
     "│ -𝐬𝐮𝐬𝐚𝐧𝐨𝐨   <𝐭>  ᯼ 𝐀𝐫𝐦𝐨𝐮𝐫    │\n"
     "│ -𝐢𝐭𝐚𝐜𝐡𝐢    <𝐭>  ᙬ 𝐒𝐡𝐚𝐝𝐨𝐰    │\n"
    ...
    ("┌─── 🎵 𝑺𝑶𝑵𝑮 & 📞 𝑽𝑶𝑰𝑪𝑬 𝑪𝑨𝑳𝑳 ──┐\n"
     "│ 𝐂𝐇𝐀𝐓 𝐒𝐄𝐍𝐃                      │\n"
     "│ -𝐚𝐝𝐝𝐬𝐨𝐧𝐠 <𝐧>  𝘙𝘦𝘱𝘭𝘺+𝘴𝘢𝘷𝘦    │\n"
     "│ -𝐬𝐨𝐧𝐠 [𝐧]     𝘚𝘦𝘯𝘥 𝘰𝘯𝘤𝘦     │\n"
     "│ -𝐬𝐨𝐧𝐠𝐬𝐩𝐚𝐦 [𝐧] 𝘓𝘰𝘰𝘱 𝘵𝘰 𝘤𝘩𝘢𝘵 │\n"
     "│ -𝐬𝐭𝐨𝐩𝐬𝐨𝐧𝐠    𝘚𝘵𝘰𝘱 𝘭𝘰𝘰𝘱      │\n"
     "│ -𝐬𝐨𝐧𝐠𝐬        𝘓𝘪𝘴𝘵 𝘭𝘪𝘣      │\n"
     "│ -𝐝𝐞𝐥𝐬𝐨𝐧𝐠 <𝐧> 𝘋𝘦𝘭𝘦𝘵𝘦         │\n"
     "│ -𝐬𝐢 [𝐧]       𝘚𝘰𝘯𝘨 𝘪𝘯𝘧𝘰    │\n"
     "│ ─────────────────────────────  │\n"
     "│ 𝐕𝐎𝐈𝐂𝐄 𝐂𝐀𝐋𝐋 (𝐧𝐞𝐞𝐝𝐬 𝐀𝐏𝐈_𝐈𝐃)   │\n"
     "│ -𝐯𝐜𝐚𝐥𝐥 [𝐧]    𝘑𝘰𝘪𝘯+𝘱𝘭𝘢𝘺    │\n"
     "│ -𝐯𝐜𝐚𝐥𝐥𝐥𝐨𝐨𝐩 [𝐧] 🔁 𝘓𝘰𝘰𝘱     │\n"
     "│ -𝐯𝐜𝐚𝐥𝐥𝐧𝐞𝐱𝐭 [𝐧] 𝘚𝘸𝘪𝘵𝘤𝘩     │\n"
     "│ -𝐯𝐜𝐚𝐥𝐥𝐬𝐭𝐨𝐩   𝘓𝘦𝘢𝘷𝘦 𝘤𝘢𝘭𝘭   │\n"
     "└───────────────────────────────┘"),
    # 20 — Tools
    ("┌────── 🛠 𝑻𝑶𝑶𝑳𝑺 ─────────────┐\n"
     "│ -𝐬𝐭𝐚𝐭𝐮𝐬       𝐍𝐂 𝐬𝐭𝐚𝐭𝐮𝐬    │\n"
    ...
    # 22 — NC1–NC115
    ("┌────── 🎯 𝑵𝑪𝟏 – 𝑵𝑪𝟏𝟏𝟓 ──────────┐\n"
     "│ 115 𝐮𝐧𝐢𝐪𝐮𝐞 𝐋𝐨𝐧𝐠-𝐍𝐂 𝐭𝐞𝐦𝐩𝐥𝐚𝐭𝐞𝐬 │\n"
     "│ ⚡ 𝐏𝐇𝐎𝐄𝐍𝐈𝐗 𝐄𝐍𝐆𝐈𝐍𝐄 · 𝐙𝐞𝐫𝐨 𝐅𝐥𝐨𝐨𝐝│\n"
     "│ ~𝟐𝟓𝟎 𝘤𝘩𝘢𝘳𝘴 · 𝟓 𝘴𝘵𝘺𝘭𝘦𝘴 · 𝟎 𝘫𝘪𝘵𝘵𝘦𝘳│\n"
     "│                                │\n"
     "│ -𝐧𝐜𝟏   ᛝ  𝐏𝐇𝐎𝐄𝐍𝐈𝐗  𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟐   ⟡  𝐏𝐇𝐎𝐄𝐍𝐈𝐗  𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟑   𒐫  𝐏𝐇𝐎𝐄𝐍𝐈𝐗  𝟎.𝟑𝟎𝐬     │\n"
     "│ -𝐧𝐜𝟒   ꧅  𝐏𝐇𝐎𝐄𝐍𝐈𝐗  𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟓   ᯼  𝐏𝐇𝐎𝐄𝐍𝐈𝐗  𝟎.𝟐𝟗𝐬     │\n"
     "│ … 𝐧𝐜𝟏–𝐧𝐜𝟏𝟏𝟓: 𝐀𝐋𝐋 𝐏𝐇𝐎𝐄𝐍𝐈𝐗   │\n"
     "│ 𝘏𝘢𝘳𝘥 𝘧𝘭𝘰𝘰𝘳 𝟎.𝟐𝟔𝘴 · 𝟑.𝟓/𝘴𝘦𝘤   │\n"
     "│                                │\n"
     "│ ✨ 𝐍𝐄𝐖 𝐔𝐍𝐈𝐐𝐔𝐄 𝐍𝐂𝐒:             │\n"
     "│ -𝐧𝐜𝟏𝟎𝟏 𒀭 𝐃𝐢𝐧𝐠𝐢𝐫  𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟏𝟎𝟐 ᚢ 𝐔𝐫𝐮𝐳    𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟏𝟎𝟑 ⁂ 𝐀𝐬𝐭𝐞𝐫𝐢𝐬𝐦 𝟎.𝟐𝟗𝐬    │\n"
     "│ -𝐧𝐜𝟏𝟎𝟒 ‼ 𝐇𝐲𝐩𝐞    𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟏𝟎𝟓 ⋯ 𝐄𝐥𝐥𝐢𝐩𝐬𝐢𝐬 𝟎.𝟐𝟗𝐬    │\n"
     "│ -𝐧𝐜𝟏𝟎𝟔 ※ 𝐑𝐞𝐟     𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟏𝟎𝟕 ❂ 𝐒𝐭𝐚𝐫    𝟎.𝟐𝟖𝐬     │\n"
     "│ -𝐧𝐜𝟏𝟎𝟖 ○ 𝐂𝐢𝐫𝐜𝐥𝐞  𝟎.𝟐𝟗𝐬     │\n"
     "│ -𝐧𝐜𝟏𝟎𝟗 ∾ 𝐖𝐚𝐯𝐞    𝟎.𝟐𝟖𝐬     │\n"
     ...
     "│ 𝐁𝐞𝐧𝐜𝐡: -𝐧𝐜𝐛𝐞𝐧𝐜𝐡             │\n"
     "└────────────────────────────────┘"),
    # 23 — Sasuke Legend NCs
    ("┌──── ⚡ 𝐓ᴏʙɪ  𝑳𝑬𝑮𝑬𝑵𝑫 𝑵𝑪𝑺 ────┐\n"
     "│ 𝟓 𝐄𝐱𝐜𝐥𝐮𝐬𝐢𝐯𝐞 𝐒𝐢𝐠𝐧𝐚𝐭𝐮𝐫𝐞 𝐍𝐂𝐬    │\n"
     "│ ⚡ 𝐏𝐇𝐎𝐄𝐍𝐈𝐗 𝐄𝐍𝐆𝐈𝐍𝐄 · 𝟎 𝐅𝐋𝐎𝐎𝐃  │\n"
     "│ ~𝟐𝟓𝟎 𝐜𝐡𝐚𝐫𝐬 · 𝟎.𝟑𝟎𝐬 · 𝐙𝐞𝐫𝐨 𝐉  │\n"
     "│                               │\n"
     "│ -𝐮𝐜𝐡𝐢𝐡𝐚    <𝐭>  ꙭ 𝐂𝐥𝐚𝐧 𝐒𝐞𝐚𝐥  │\n"
     "│ -𝐜𝐡𝐢𝐝𝐨𝐫𝐢   <𝐭>  ᛠ ⚡ 𝐁𝐥𝐚𝐝𝐞   │\n"
     "│ -𝐚𝐦𝐚𝐭𝐞𝐫𝐚𝐬𝐮 <𝐭>  ꫫ 🔥 𝐅𝐥𝐚𝐦𝐞𝐬 │\n"
     "│ -𝐤𝐢𝐫𝐢𝐧     <𝐭>  ᙘ 🐉 𝐃𝐫𝐚𝐠𝐨𝐧  │\n"
     "│ -𝐜𝐮𝐫𝐬𝐞𝐦𝐚𝐫𝐤 <𝐭>  ꩤ 🐍 𝐂𝐮𝐫𝐬𝐞  │\n"
     "│                               │\n"
     "│ 𝘗𝘩𝘰𝘦𝘯𝘪𝘹: 𝟏 𝘴𝘦𝘯𝘥/𝟎.𝟑𝟎𝘴 = 𝟎 𝘧𝘭𝘰𝘰𝘥│\n"
     "│ 𝘗𝘦𝘳-𝘣𝘰𝘵 𝘴𝘬𝘪𝘱 · 𝘏𝘢𝘳𝘥 𝘧𝘭𝘰𝘰𝘳   │\n"
     "│                               │\n"
     "│ 𝐄𝐱: -𝐮𝐜𝐡𝐢𝐡𝐚 𝐓𝐎𝐁𝐈          │\n"
     "│ 𝐄𝐱: -𝐜𝐡𝐢𝐝𝐨𝐫𝐢 𝐋𝐈𝐍𝐆           │\n"
     "│ -𝐦𝐞𝐧𝐮 𝟏𝟒 → 𝐎𝐥𝐝 𝐄𝐥𝐢𝐭𝐞 𝐍𝐂𝐬   │\n"
     "└───────────────────────────────┘"),
]
...
# ── Sasuke Legend NCs (5 new) — PRIME ENGINE ─────────────────────────────────
...
async def cmd_allcmds(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "\n"
        "  🔥 𝐍𝐂 𝐂𝐎𝐑𝐄\n"
        "  -𝐧𝐜  -𝐬𝐧𝐜  -𝐚𝐧𝐬𝐡𝐠𝐨𝐝  -𝐬𝐚𝐬𝐮𝐤𝐞𝐠𝐨𝐝\n"
        "  -𝐬𝐚𝐬𝐮𝐤𝐞𝟏  -𝐭𝐫𝐢𝐨𝐠𝐨𝐝  -𝐬𝐢𝐥𝐤𝐧𝐜  -𝐜𝐡𝐮𝐝\n"
        "  -𝐛𝐨𝐥𝐝𝐧𝐜  -𝐜𝐮𝐫𝐬𝐢𝐯𝐞𝐧𝐜  -𝐢𝐭𝐚𝐥𝐢𝐜𝐧𝐜  -𝐰𝐚𝐯𝐞𝐧𝐜\n"
        "  -𝐭𝐨𝐛𝐢𝐧𝐜  -𝐯𝐢𝐥𝐥𝐚𝐢𝐧𝐧𝐜  -𝐫𝐚𝐧𝐝𝐨𝐦𝐜𝐨𝐝  -𝐠𝐨𝐝𝐜𝐨𝐝\n"
...
@_guard
async def cmd_sasukencs(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.message or update.edited_message
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx) or "SASUKE"
    idx = [0]
    factories = [_sasuke_factory(txt, i) for i in range(10)]
    def _cycling_factory():
        f = factories[idx[0] % len(factories)]
        idx[0] += 1
        return f()
    await _start_nc(msg, cid, _cycling_factory, f"⚡ All Sasuke NCs")
...
@_guard
async def cmd_aizennc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.message or update.edited_message
    if not msg:
        return
    txt = _txt_arg(ctx) or "AIZEN"
    await _start_nc(msg, msg.chat_id, _aizen_factory(txt), "♛ AIZEN NC")

@_guard
async def cmd_villainnc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.message or update.edited_message
    if not msg:
        return
    txt = _txt_arg(ctx) or "VILLAIN"
    await _start_nc(msg, msg.chat_id, _villain_factory(txt), "𖤍 VILLAIN NC")
...
@_guard
async def cmd_anshgod(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.message or update.edited_message
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx)
    if not txt:
        await _reply(msg, "𝐔𝐬𝐞: -anshgod <text>")
        return
    bots = _bots()
    if not bots:
        await _reply(msg, "⚡ 𝐍𝐨 𝐛𝐨𝐭𝐬 𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞!")
    ...
    _nc_info[cid] = {"engine": "ANSHGOD", "text": txt, "start_t": time.monotonic()}
...
@_guard
async def cmd_sasuke1(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.message or update.edited_message
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx)
    if not txt:
        await _reply(msg, "𝐔𝐬𝐞: -sasuke1 <text>")
        return
    bots = _bots()
    if not bots:
        await _reply(msg, "⚡ 𝐍𝐨 𝐛𝐨𝐭𝐬 𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞!")
    ...
    _nc_info[cid] = {"engine": "SASUKE1", "text": txt, "start_t": time.monotonic()}
...
@_guard
async def cmd_sasukegod(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.message or update.edited_message
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx)
    if not txt:
        await _reply(msg, "𝐔𝐬𝐞: -sasukegod <text>")
        return
    bots = _bots()
    if not bots:
        await _reply(msg, "⚡ 𝐍𝐨 𝐛𝐨𝐭𝐬 𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞!")
        return
    gap = _nc_send_gap if _nc_send_gap is not None else 0.25
    _nc_info[cid] = {"engine": "SASUKEGOD", "text": txt, "start_t": time.monotonic()}

    async def _run(stop_ev):
        try:
            await _god_engine(cid, bots, stop_ev, _sasuke_factory(txt, 0))
        finally:
            _nc_info.pop(cid, None)
...
_NC_DELAY_DEFAULTS: Dict[str, float] = {
    "blaze": 0.01, "surge": 0.001, "god": 0.001,
    "stagger": 0.01, "silk": 0.01, "trio": 0.1, "chud": 0.20,
    "mgcnc": 0.02,
}
...
_NC_DELAY_DEFAULTS.update({
    "nc101": 0.2, "nc102": 0.3, "nc103": 0.03, "nc104": 0.01, "nc105": 0.01,
    "nc106": 0.03, "nc107": 0.8, "nc108": 0.2, "nc109": 0.2, "nc110": 0.9,
    "nc111": 0.2, "nc112": 0.2, "nc113": 0.9, "nc114": 0.2, "nc115": 0.9,
})
...
async def cmd_ncdel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx) or "SASUKE LING"
    ncdel_chats.add(cid)
...
async def cmd_ncwar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg:
        return
    cid = msg.chat_id
    txt = _txt_arg(ctx) or "SASUKE LING WAR"
...
async def cmd_song(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        ...
        await _reply(msg, "⚠️ 𝐅𝐚𝐢𝐥𝐞𝐝 𝐭𝐨 𝐬𝐞𝐧𝐝 𝐬𝐨𝐧𝐠 — 𝐛𝐨𝐭 𝐦𝐚𝐲 𝐧𝐞𝐞𝐝 𝐚𝐝𝐦𝐢𝐧")
...
async def main():
    ...
    if _hard_stop:
        print("[✓] TOBI V12stopped.")
        return False       # caller exits

    print("[✓] TOBI V12restarting…")
